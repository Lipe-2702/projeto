...
@Service
public class TransacaoBancariaContaMembro {
    @Autowired
    ...
    public void depositar(Integer numConta, Integer senha, BigDecimal valor) throws Exception {
        ContaMembro contaMembroEncontrada = membroService.listarContaIgreja(numConta, senha).orElseThrow(() -> new Exception("Conta não encontrada"));
        try {
            BigDecimal saldoAtual = contaMembroEncontrada.getSaldo();
            BigDecimal novoSaldo = saldoAtual.add(valor);
            contaMembroEncontrada.setSaldo(novoSaldo);
        } catch (Exception e) {
    ...
    public void transferir(Integer numeroDaContaOrigem, Integer numeroDaContaDestino, Integer senha, BigDecimal valor) throws Exception {
        ContaMembro contaMembroEncontrada = membroService.listarContaIgreja(numeroDaContaOrigem, senha).orElseThrow(() -> new Exception("Conta não encontrada"));
        ContaIgreja contaIgrejaEncontrada = igrejaService.listarContaIgreja(numeroDaContaDestino, senha).orElseThrow(() -> new Exception("Conta não encontrada"));
        try {
            BigDecimal saldoAtual = contaMembroEncontrada.getSaldo();
            if (valor.compareTo(BigDecimal.ZERO) <= 0) {
